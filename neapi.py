...
NEAPI_METHOD_GET = 0x0
NEAPI_METHOD_POST_NORMAL = 0x1
NEAPI_METHOD_POST_ENCRYPTED = 0x2
NEAPI_METHOD_GET_AND_POST_ENCRYPTED = 0x3
NEAPI_BRS = {
    ('b', 96000),
    ('m', 128000),
    ('l', 192000),
...
URL_NEAPIS = {
    ...
    "getFMSonglist" : 
        [("/weapi/v1/radio/get", NEAPI_METHOD_POST_ENCRYPTED, {
            ...
            'songlistSongsID2': lambda obj: [map(lambda x:song.get(x,{}).get('id'), [u"bMusic",u"lMusic", u"mMusic", u"hMusic"]) for song in obj['data']],
            'songID': lambda obj: obj['id'],
            'songID2': lambda obj: map(lambda x:obj.get(x,{}).get('id'), [u"bMusic",u"lMusic", u"mMusic", u"hMusic"]),
            'songAlbumName': lambda obj: obj['album']['name'],
            'songAlbumID': lambda obj: obj['album']['id'],
    ...
    "getAlbum" : 
        [("/api/album/%s", NEAPI_METHOD_GET, 
        {
            ...
            'albumSongsID2': lambda obj: [map(lambda x:song.get(x,{}).get('dfsId'), [u"bMusic",u"lMusic", u"mMusic", u"hMusic"]) for song in obj['album']['songs']],
        ...
        ("/weapi/v1/album/%s", NEAPI_METHOD_POST_ENCRYPTED, 
        {
            ...
            'albumSongsName' : lambda obj: [song["name"] for song in obj['album']['songs']],
            'albumSongsID' : lambda obj: [song["id"] for song in obj['album']['songs']],
            'albumSongsID2': lambda obj: [map(lambda x:song.get(x,{}).get('fid'), [u"b",u"l", u"m", u"h"]) for song in obj['album']['songs']],
    ...
    "getSongsURL" : 
        ...
        {
            'urls': lambda obj: [x['url'] for x in obj['data']],
        }
        )
        ],
    "getLyric" : 
        ...
        {
            'lyric': lambda obj: x.get("lrc",{}).get("lyric"),
            'translatedLyric': lambda obj: x.get("tlyric",{}).get("lyric"),
        }
        )
        ],
    "getSongComments" : 
...
def readWriteCallback(readIO, writeIO, callbackFunc, blockSize, totalSize):
    receivedBlocks = 0
    receivedSize = 0
    if callbackFunc: callbackFunc(receivedBlocks, blockSize, receivedSize, totalSize)
...
class NEApi:
    ...
    def _myHTTP(self, url, method = 'GET', params = '', query  = '', header = {}):
        ...
        return rawHTTP(URL_HOST, url, method, params, query, newHeader, self.httpLog, self.config["readCallback"])
    ...
    def _mySubmit(self, APIURL, params, *urlFormat):
        ...
        postParams = csrfParam
        getQuery = csrfParam
        if APIURL[1] == NEAPI_METHOD_GET:
            getQuery.update(params)
            return self._myHTTP(APIURL[0], 'GET', {}, getQuery, {})
        if APIURL[1] == NEAPI_METHOD_POST_NORMAL:
            postParams.update(params)
            return self._myHTTP(APIURL[0], 'POST', postParams, getQuery, {})
        if APIURL[1] == NEAPI_METHOD_POST_ENCRYPTED:
            postParams.update(params)
            return self._encHTTP(APIURL[0], postParams, getQuery, {})
    ...
    def _loadCookie(self):
        ...
        self.setCookie(cookie)
        d(u"cookie:%s" % self.localData['cookie'])
    ...
    def getUserSonglistList(self, uid = 0, offset = 0, limit = 1000):
        ...
        currAPIURL = currAPIURL[max(currAPIVersion, len(currAPIURL) - 1)]
        currDict = {
            'offset' : offset,
            'limit' : limit,
            'uid' : uid,
...
#modify
    ...
    def getSongsURL_direct(self, songIDList, br, **kwargs):
        ...
        if not kwargs['forceOldAPI']:
            urlList, urlListFunc = self.getSongsURL(songIDList, br)
            urlList = getItem(urlListFunc, urlList, "urls")
        ...
        for i, url in enumerate(urlList):
            if url is None:
                # get url by album
                if songDict[i].has_key("al"):
                    albumID = songDict[i]["al"]["id"]
                elif songDict[i].has_key("album"):
                    albumID = songDict[i]["album"]["id"]
                else:
                ...
                albumSongIndex = albumSongsID.index(songIDList[i])
                availableBrs = [i for i, entry in enumerate(albumSongsDfsID[albumSongIndex]) if entry is not None]
                ...
                dfsID = albumSongsDfsID[closestBrNum]
                if not dfsID:
                    self.apiLog.error("No DFSID for Song %s !!" % songIDList[i])
                else:
                    mp3URLEncryptor = NEEncrypt()
                    mp3URLEncryptor.loadDfsID(dfsID)
                    urlList[i] = mp3URLEncryptor.getMP3URL()
                    if not isURLAvailable(urlList[i]):
                        self.apiLog.error("Failure getting old-api url  for song %s !!" % songIDList[i])
                        urlList[i] = None
        return urlList
