...
if MAIN_ON_PC:
    MAIN_DEBUG_FOLDER = os.path.abspath(".\\neteaseDebug")
    MAIN_CURR_FOLDER = os.path.abspath(".\\")
    MAIN_NETEASE_FOLDER = os.path.abspath(".\\netease")
    MAIN_NETEASE_MUSIC_FOLDER = os.path.abspath(".\\neteaseMusic")
    sys.path.append(MAIN_CURR_FOLDER)

if MAIN_ON_PYCON:
    MAIN_DEBUG_FOLDER = os.path.abspath("e:\\neteaseDebug2")
    MAIN_CURR_FOLDER = os.path.abspath("e:\\python")
    MAIN_NETEASE_FOLDER = os.path.abspath("e:\\netease2")
    MAIN_NETEASE_MUSIC_FOLDER = os.path.abspath("e:\\music")
    if sys.path[1] != MAIN_CURR_FOLDER:
        sys.path[1:1] = [MAIN_CURR_FOLDER]
    os.chdir(MAIN_CURR_FOLDER)
...
apiLogHandler = logging.FileHandler(pj(MAIN_DEBUG_FOLDER, "apilog.txt"), mode="w")
apiLogHandler.setLevel(logging.WARNING)

httpLogHandler = logging.FileHandler(pj(MAIN_DEBUG_FOLDER, "httplog.txt"), mode="w")
httpLogHandler.setLevel(15)

setUtilLogHandler(logHandler)
...
def definePlots():
    ...
    global buf, plots, img, screen_x, screen_y
    buf = graphics.Image.new(CANVASSIZE)
    plots = range(0,100)
    
    logs("start defining plots")
    plots[1] = screen_y /2
    ...
    logs("the type of plot14 : %s" % type(plots[14]))

    for i, x in enumerate(plots):
        if isinstance(x, float):
            plots[i] = int(x)
        if isinstance(x, tuple):
            plots[i] = tuple(int(y) for y in x)
    ...
    logs("start loading images")
    img[0] = graphics.Image.open( pj(MAIN_CURR_FOLDER,'\\pic\\pic.bmp')).resize((plots[0],plots[0]))
    img[1] = img[0]
    img[2] = graphics.Image.new((plots[0],plots[0]),mode='1')
    img[3] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_disc.png')).resize((plots[1] ,plots[1]))
    img[4] = graphics.Image.new((plots[1],plots[1]),mode='1')
    img[5] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_needle.png')).resize((plots[2] ,plots[2] * 51 / 39))
    img6_ = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_needle_.png')).resize((plots[2] ,plots[2] * 51 / 39))
    ...
    img[10] = img[1].resize((16,16)).resize(CANVASSIZE)
    img[11] = graphics.Image.new(CANVASSIZE, 'L')
    img[11].clear(0x303030)
    img[12] = img[1].resize((22,22)).resize(CANVASSIZE)
    img[13] = graphics.Image.new(CANVASSIZE, 'L')
    ...
    logs("start loading images 21")
    img[21] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_love.bmp'))
    img[22] = graphics.Image.new(img[21].size, 'L')
    img[22].load(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_love_.bmp'))
    img[21] = img[21].resize((plots[16], plots[16]))
    img[22] = img[22].resize((plots[16], plots[16]))
    img[23] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_loved.bmp'))
    img[24] = graphics.Image.new(img[23].size, 'L')
    img[24].load(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_loved_.bmp'))
    img[23] = img[23].resize((plots[16], plots[16]))
    img[24] = img[24].resize((plots[16], plots[16]))
    logs("start loading images 25")
    img[25] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_loop.bmp'))
    img[26] = graphics.Image.new(img[25].size, 'L')
    img[26].load(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_loop_.bmp'))
    img[25] = img[25].resize((plots[16], plots[16]))
    img[26] = img[26].resize((plots[16], plots[16]))
    logs("start loading images 27")
    img[27] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_one.bmp'))
    img[28] = graphics.Image.new(img[27].size, 'L')
    img[28].load(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_one_.bmp'))
    img[27] = img[27].resize((plots[16], plots[16]))
    img[28] = img[28].resize((plots[16], plots[16]))
    
    img[29] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_shuffle.bmp'))
    img[30] = graphics.Image.new(img[29].size, 'L')
    img[30].load(pj(MAIN_CURR_FOLDER, '\\pic\\play_icn_shuffle_.bmp'))
    img[29] = img[29].resize((plots[16], plots[16]))
    img[30] = img[30].resize((plots[16], plots[16]))
    
    img[31] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\list_detail_icn_fav.bmp'))
    img[32] = graphics.Image.new(img[31].size, 'L')
    img[32].load(pj(MAIN_CURR_FOLDER, '\\pic\\list_detail_icn_fav_.bmp'))
    img[31] = img[31].resize((plots[16], plots[16]))
    img[32] = img[32].resize((plots[16], plots[16]))
    
    img[33] = graphics.Image.open(pj(MAIN_CURR_FOLDER, '\\pic\\list_detail_icn_faved.bmp'))
    img[34] = graphics.Image.new(img[33].size, 'L')
    img[34].load(pj(MAIN_CURR_FOLDER, '\\pic\\list_detail_icn_faved_.bmp'))
    img[33] = img[33].resize((plots[16], plots[16]))
    img[34] = img[34].resize((plots[16], plots[16]))
    
    logs("start loading images 35")
    #img35是一个List，包括旋转过的专辑封面图片
    img[35] = range(rotatingImages)
    
    #img36是进度条
    img[36] = graphics.Image.new((screen_x, 5))
    img[36].clear(0x000000)
    img[37] = graphics.Image.new((screen_x, 5),  'L')
    img[37].clear(0xFFFFFF)
    buf = graphics.Image.new(CANVASSIZE)
    
FONT_MEDIUM = int((screen_x + screen_y)/ 2 * 20 / 300)
...
def initDraw():
    ...
    buf.text((plots[10][0],plots[10][1]+FONT_MEDIUM + 3), cn('网易云音乐'),fill=0xffffff,font=("normal",FONT_MEDIUM,graphics.FONT_BOLD|graphics.FONT_ANTIALIAS))
    buf.text((plots[10][0]+15,plots[10][1]+FONT_MEDIUM +FONT_SMALL + 6), cn('请选择左键菜单中的启动连接') ,fill=0xffffff,font=("normal",FONT_SMALL,None))
    buf.text((plots[10][0]+15,plots[10][1]+FONT_MEDIUM+FONT_SMALL+FONT_SMALL + 9), cn('选项-设置-帮助关于 可以看使用帮助'),fill=0xffffff,font=("normal",FONT_SMALL,None))
    lyricBlit(currLrc)
    redraw()
...
def drawRotation():
    ...
    for i in range(img[1].size[0]):
        ...
        # logs("About to ao_yield in function %s" % sys._getframe().f_code.co_name)
        e32.ao_yield()
...
def redrawPosition():
    ...
    if not('npl' in globals()) or not(npl.player):
        return
    if npl.player.state() == audio.ENotReady:
        return
    ...
    if npl.player != False and npl.player.state() == audio.EPlaying and not(toastVisibility) and foreground:
        aotimer3.cancel()
        aotimer3.after(1, redrawPosition)
...
def refreshButtonsStatus():
    ...
    position = position / 1000000L
    logs('position : %s' % position)
    likeResult, likeFunc = nma.NE_likeFMSong(npl.songList[songIndex], "itembased", True, position)

    if likeResult.get('songs') == []:
        #表示之前没有喜欢
        likeResult, likeFunc = nma.NE_likeFMSong(npl.songList[songIndex], "itembased", False, position)
        liked = False
    else:
        #之前已经喜欢了
        liked = True
...
def redrawSong(pl):
    ...
    # buf.blit(img9,target = (5,20))
    buf.blit(img[12],target = (0,0), source=((0,0),(plots[4][0],plots[4][1] *0.18)),mask=img[13])
    logs("panic checkpoint b5")

    redrawPosition()
    drawButtons()
    buf.text((plots[10][0],plots[10][1]+FONT_MEDIUM + 3), s(d(songFunc, songDict, "songName")),fill=0xffffff,font=("normal",FONT_MEDIUM,graphics.FONT_BOLD|graphics.FONT_ANTIALIAS))
    buf.text((plots[10][0]+15,plots[10][1]+FONT_MEDIUM+FONT_SMALL + 6), s(' & '.join(d(songFunc, songDict, "songArtistsName"))) ,fill=0xffffff,font=("normal",FONT_SMALL,None))
    buf.text((plots[10][0]+15,plots[10][1]+FONT_MEDIUM+FONT_SMALL + FONT_SMALL +9), cn(s(d(songFunc, songDict, "songAlbumName"))) + cn(' #') + str(songDict['no']),fill=0xffffff,font=("normal",FONT_SMALL,None))
    
    logs("panic checkpoint b6")
    #e32.ao_sleep(0, drawRotation)
    redrawPosition()
    redraw()
    logs("panic checkpoint b7")
...
def setToastText(strList, fontScale = 0.3, secondFontScale = 0.2):
    ...
    img[14].text((plots[11] *0.18, plots[12] * ((1-textHeight)/2 + fontScale)), s(strList[0]),fill=0xffffff,font=("normal",plots[12]*fontScale,None))
    for k,v in enumerate(strList[1:]):
        logs(u"toastLine:%s"%s(v))
        img[14].text((int(plots[11] *0.22), int(plots[12] * ((1-textHeight)/2 + fontScale + secondFontScale * 1.1 * (k+1))) ), s(v) ,fill=0xffffff,font=("normal",int(plots[12]*secondFontScale),None))
    refreshToast()
...
def downProgWrapper(strList_):
    ...
    def downProgUpdater(blocknum, blocksize, totalsize):
        ...
        #logs("About to ao_yield in function %s" % sys._getframe().f_code.co_name)
        #e32.ao_sleep(PY2_BUG_DELAY_TIME)
        e32.ao_yield()
        #logs("End ao_yield-ing in function %s" % sys._getframe().f_code.co_name)
...
def pauseCont():
    if 'npl' in globals():
        ...
        redrawPause(npl)
        aotimer3.cancel()
        aotimer3.after(0, redrawPosition)
        # aotimer3.after(0, loopRotate)
...
def nextSong(manual = True):
    ...
    logs('nextSong, sSH: '+repr(songSwitchHangedon))
...
def prevSong():
    global checkLI, songSwitchHangedon
    logs('prevSong, sSH: '+repr(songSwitchHangedon))
    if not songSwitchHangedon:
        if 'npl' in globals():
            songSwitchHangedon = True
            if not(currentFM):
...
def playCallback():
    ...
    if (len(npl.songList) > (npl.songIndex) and npl.songList[npl.songIndex] != currentPlaying) or not(bool(npl.songList[npl.songIndex])) or currentPlaying == -1:
        ...
        aotimer3.cancel()
        refreshButtonsStatus()
        logs('panic checkpoint a3')
        redrawSong(npl)
        logs('panic checkpoint a4')
        aotimer2.cancel()
        if npl.isScroll:
            refreshLyric()
        refreshCurrPlaylist()
...
def songCallback(prevState, nowState, errCode):
    ...
    if prevState == audio.EPlaying and nowState == audio.EOpen:
        ...
        aotimer3.after(0, redrawPosition)
        #aotimer3.after(0, loopRotate)
        aotimer_tmp = e32.Ao_timer()
        aotimer_tmp.after(0, playCallback)
...
def printHandler():
    logs('tab[2] exit_key_handler: (variable)' + repr(tabKey[2][1]))
    logs('tabKey: ' + repr(tabKey))
    logs('now exit_key_handler: (real)' + repr(appuifw.app.exit_key_handler))
...
configFileName = ""
#读入接入点，没有就创建一个空文件
currConfig = {'ap': -1,  'username': '', 'password': '', 'cellphone': False, 'volume':1, 'bps':"m", 'api':1}
def loadConfig():
    global currConfig, configFileName
    configFileName = pj(MAIN_NETEASE_FOLDER, 'data\\config.txt')
    logs("configFileName : %s" % configFileName)
    if not os.path.exists(configFileName):
        writeConfig()
    else:
        file1=open(configFileName,'r')
        fileRead = file1.read()
        currConfig = json.loads(fileRead)
        file1.close()
    return currConfig
    
def writeConfig():
    file1=open(configFileName,'wb')
...
def checkComm():
    ...
    #if currAp != -1:
    if True:
        #apo = btsocket.access_point(int(currAp))
        #logs(u"access point : %s" % apo)
        #btsocket.set_default_access_point(apo)
        #apo.start()
        conn1 = httplib.HTTPConnection('music.163.com', 80)
...
#选择接入点
def selAp(tip=False):
    currAp = btsocket.select_access_point()
    if currAp==None:
...
def setBps():
    ...
    bpsValues = ['b', 'l', 'm', 'h']
    bpsTexts = [u'96Kbps', u'128Kbps', u'192Kbps', u'320Kbps']
...
def aboutText():
    appuifw.query(cn('网易云音乐PyS60 2.0.0支持专版 ,v0.1.0\nBy 顺'),'query')

def selAPI():
    appuifw.note(cn("现在只能使用新版 API 了。"))
    return
...
progress = 0
globalInfoPopup = appuifw_old.InfoPopup()

def downProg_small(n,s,r,t):
    global progress, globalInfoPopup
    pc = r*100/t
    if pc > 100:
        pc=100
    if pc < progress:
        progress = pc - 5
    if pc - progress >= 5:
        globalInfoPopup.show(cn('↓  ') + u'%s %%' % pc,(0,0), 200,0, appuifw_old.EHCenterVTop)
        progress = pc

    e32.ao_yield()
    #e32.ao_sleep(0, yieldFunc)
    #e32.ao_sleep(PY2_BUG_DELAY_TIME)
        ...
        #     if ''.join(stack).find("playCallback") != -1:
        #         logs("in playCallback. bug trigging")
        #     #appuifw_old.InfoPopup()
        #     else:
        #         e32.ao_yield()
...
def login():
    ...
    nma = neapi.NEApi(
        {
            ...
            "cellphone" : loginUsername,
            "isPhone" : loginCellphone,
            "HTTPLogHandler" : httpLogHandler,
            "LogHandler" : apiLogHandler,
            "LoadLocalDataFunc" : loadLocalDataF,
            "WriteLocalDataFunc" : writeLocalDataF,
            "apiVersion" : 1,
            "readCallback": downProg_small
        })
    ...
    while True:
        ...
        while nma.needCaptcha:
            captchaURL = nma.getCaptchaURL()
            captchaPath = pj(MAIN_NETEASE_FOLDER, ".\\captcha.png")
            import urllib
            urllib.urlretrieve(captchaURL, captchaPath)
            captcha = appuifw.query(s("请输入%s验证码："%captchaPath), 'text', "")
            if(captcha == ''):
                #User Cancellation
                userCancelLogin = True
                appuifw.note(s("登录已取消"))
                continue
            nma.sendCaptcha(captcha)
        ...
        try:
            ...
            if clientToken == '' or clientToken is None:
                ...
                appuifw.note(s("登录已取消"))
                continue
            nma.setClientToken(clientToken)
            continue
        break
    if not nma.logined:
        return -1
...
def getUserPLs():
    ...
    userPlaylists, userPlaylistsF = nma.getUserSonglistList()
    userPlaylists = userPlaylists["playlist"]
    ...
    try:
        ...
        if playlistIDInput == -1:
            ...
            if playlistDict_['trackCount'] > 70:
                if not(appuifw.query(cn('歌单过大，若点播需要较长的时间载入和分析数据，几百首歌的歌单甚至可能需要几十分钟。确定点播吗？'), 'query')):
                    return
                offset = appuifw.query("最多加载几首歌？", "number", 60)
                if offset is None:
                    return
                limit = appuifw.query("从第几首开始加载？按新到旧排序", "number", 0)
                if limit is None:
                    return
        ...
        global songFunc
        playlistR, playlistF = nma.getSonglist(playlistId, limit, offset, False)
        playlistDict = playlistR['playlist']
        songFunc = playlistF

        toastNext(toastTitle, toastList)
        npl.updateListByDict(playlistDict['tracks'], playlistF)
...
def dailyRec(startNow = True):
    global toastIndex, songFunc, playlistDict
    ...
    if npl.player:
        pause()
    toastNext(toastTitle, toastList)
    playlistR, playlistF = nma.getRecommendationSonglist()
    playlistDict = playlistR['recommend']
    songFunc = playlistF
    toastNext(toastTitle, toastList)
    npl.updateListByDict(playlistDict, songFunc)
...
def loadSong():
    ...
    #if not songSwitchHangedon:
    #@this will prevent loadSong after error occurred
    if True:
        if 'npl' in globals():
            songSwitchHangedon = True
...
def playlistDetail(whetherGetSongs = False):
    ...
    if whetherGetSongs:
        ...
        playlistSongsDict = nma.getSonglist(playlistDict['id'])[0]['playlist']['tracks']
        appuifw.note(cn('获取曲目信息成功'))
        for i, song in enumerate(playlistSongsDict):
            plDetailTitles.append(unicode(i))
            
            tmpPLDetailContent = cn(song['name']) + u' - ' + cn(song['ar'][0]['name'])
            if len(song['ar']) > 1: tmpPLDetailContent += cn('等')
...
def createNpl():
    global npl
    npl = playlist.Playlist([], nma, 0, 73, currConfig['bps'], currConfig['volume'], False, logHandler, pj(MAIN_NETEASE_FOLDER, "lyric\\%s.lyric.json"), pj(MAIN_NETEASE_FOLDER, "pic\\%s.cover.png"), pj(MAIN_NETEASE_FOLDER, "cache\\%s_%s.mp3"), pj(MAIN_NETEASE_MUSIC_FOLDER, "\\%s-%s.mp3"), pj(MAIN_NETEASE_MUSIC_FOLDER, "\\%s-%s.lrc"))
    npl.updateListByDict(playlistDict, songFunc)
    npl.setPlaylistName(cn('每日推荐'))
...
def favorPlaylist():
    ...
    if plId==None: return
    favorResult, favorF = nma.NE_subscribeSonglist(plId)
...
def search():
    ...
    searchResult = nma.compat_search(queryString, queryType)[queryKey]
...
def songInfo(songIndex = -1):
    ...
    try:
        ...
        duration = d(songFunc, songDict, "songDuration")
    try:
        ...
        if 'reason' in songDict.keys():
            ...
            reason = '无'
        alias = u', '.join(d(songFunc, songDict, "songAlias"))
    
        artists = '& '.join(d(songFunc, songDict, "songArtistsName"))
        songInfoTitles = ['曲名','歌手','专辑','轨道序号','长度','ID','推荐理由','别名']
        songName = d(songFunc, songDict, "songName")
        songAlbumName = d(songFunc, songDict, "songAlbumName")
        songTrackNo = d(songFunc, songDict, "songTrackNo")
        songID = d(songFunc, songDict, "songID")

        songInfoContents = [songName,artists,songAlbumName,songTrackNo,duration,songID,reason,alias]
...
def commOper(operNo):
    try:
        ...
        if operNo == 3:
            CommIndex = commListbox.current()
            favorSuc, favorMsg_ = nma.compat_favorComment(CommList[CommIndex]['commentId'], threadId, not(CommList[CommIndex]['liked']))
        ...
        import traceback
        mainLogger.error(traceback.format_exc())
        appuifw.note(s("发生错误。若程序不能正常运行，请重新启动程序。"))
        appuifw.note(s("查看文件夹E:\\NeteaseDebug2下的日志文件来排查错误。"))
...
def viewCommWrapper(hot=False, songid=-1, threadid='_', additionalParam = ''):
    def viewComm():
        ...
        (hotCommList, ordCommList, totalnum, havemore) = nma.compat_getComments(commViewingId, threadId, commOffset_, False, commLimit)
...
def refreshCurrPlaylist(busy = False, songIndex = -1):
    ...
    logs("before creeating items")
    currPlaylist = [appuifw.Item(cn(x['name'])) for x in npl.listdetail]
    logs("after creeating items")
    
    symbol = (cn('→ '), cn('◎ '))[busy]
    if songIndex == -1:
        songIndex = npl.songIndex
    logs("before updating lists")
        
    currPlaylist[songIndex].title = symbol + currPlaylist[songIndex].title;logs("1")
    currPlaylistListbox.begin_update();logs("2")
    currPlaylistListbox.clear();logs("3")
    for x in currPlaylist:
        currPlaylistListbox.append(x)
    currPlaylistListbox.set_current(songIndex);logs("4")
    currPlaylistListbox.end_update();logs("5")
    logs("after updating lists")
...
def toastNext(toastTitle, toastList):
    ...
    if currToastList[1] != u'':currToastList[1] = currToastList[1] + u'OK'
