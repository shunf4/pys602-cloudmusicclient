...
def errorDisplayFunc(sth):
    aoErr = e32.Ao_lock()
    bodyBack = appuifw.app.body
    exitBack = appuifw.exit_key_handler
    appuifw.app.body = appuifw.Text_display(text=s(sth), skinned=True)
    def restoreError():
        aoErr.signal()
        appuifw.app.body=bodyBack
        appuifw.exit_key_handler=exitBack
    appuifw.exit_key_handler = restoreError
    aoErr.wait()
...
if MAIN_ON_PYCON:
    ...
    MAIN_NETEASE_MUSIC_FOLDER = os.path.abspath("e:\\neteaseMusic")
...
newStdErr = tools.Stderr(sys.stderr, mainLogger, errorDisplayFunc)
sys.stderr = newStdErr
appuifw.note(s(repr(sys.stderr)))
raise
...
FONT_MEDIUM = (screen_x + screen_y)/ 2 * 16 / 300
FONT_SMALL = int(FONT_MEDIUM * 0.7)
...
def initDraw():
    ...
    buf.text((plots[10][0],plots[10][1]+17), cn('网易云音乐'),fill=0xffffff,font=("normal",FONT_MEDIUM,graphics.FONT_BOLD|graphics.FONT_ANTIALIAS))
    buf.text((plots[10][0]+15,plots[10][1]+17+12), cn('请选择左键菜单中的启动连接') ,fill=0xffffff,font=("normal",FONT_SMALL,None))
    buf.text((plots[10][0]+15,plots[10][1]+17+12+12), cn('选项-设置-帮助关于 可以看使用帮助'),fill=0xffffff,font=("normal",FONT_SMALL,None))
    lyricBlit(currLrc)
    redraw()
...
def redrawSong(pl):
    ...
    buf.text((plots[10][0],plots[10][1]+17), cn(songDict['name']),fill=0xffffff,font=("normal",16,graphics.FONT_BOLD|graphics.FONT_ANTIALIAS))
    buf.text((plots[10][0]+15,plots[10][1]+17+12), cn(' & ').join([cn(artist['name']) for artist in songDict['artists']]) ,fill=0xffffff,font=("normal",11,None))
    buf.text((plots[10][0]+15,plots[10][1]+17+12+12), cn(songDict['album']['name']) + cn(' #') + str(songDict['no']),fill=0xffffff,font=("normal",11,None))
    redraw()
...
def aboutText():
    appuifw.query(cn('网易云音乐Python S60 Alpha 0.0.8版\nBy 顺'),'query')
...
loadLocalDataF, wrtieLocalDataF = localDataFuncWrapper(pj(MAIN_NETEASE_FOLDER, "localdata.txt"), {"cookie": "", "clientToken" : "1_1UlfKAX7rcMafd1M0ndPFJPGITMgOexy_IZ8ivs4nXAGjweA4z6VEtLdEP143Y5Bg"})

def downProg_small(n,s,t):
    global progress
    pc = n*s*100/t
    if pc > 100:
        pc=100
...
def loadPlaylist(startNow = True, playlistIDInput = -1):
    ...
    if npl.player:
        pause()
    toastNext(toastTitle, toastList)
    playlistR, playlistF = nma.getSonglist(playlistId)
...
def createNpl():
    global npl
    npl = playlist.Playlist([], nma, 0, 73, currConfig['bps'], currConfig['volume'], False, logHandler, pj(MAIN_NETEASE_FOLDER, "lyric\\%s.lyric.json"), pj(MAIN_NETEASE_FOLDER, "pic\\%s.cover.png"), pj(MAIN_NETEASE_MUSIC_FOLDER, "\\%s_%s.mp3"))
...
def songInfo(songIndex = -1):
    ...
    alias = u', '.join([cn(x) for x in d(songFunc, songDict, "songAlias")])
    artists = cn('& '.join(d(songFunc, songDict, "songArtistsName")))
    
    songInfoTitles = ['曲名','歌手','专辑','轨道序号','长度','ID','推荐理由','别名','MP3路径','专辑图路径','歌词路径']
    songInfoContents = [d(songFunc, songDict, "songName"),artists,d(songFunc, songDict, "songAlbumName"),d(songFunc, songDict, "songTrackNo"),d(songFunc, songDict, "songID"),reason,alias,npl.songPath,npl.picPath,npl.lrcPath]
