...
def initDraw():
    ...
    buf.text((plots[10][0],plots[10][1]+17), cn('网易云音乐'),fill=0xffffff,font=("normal",16,graphics.FONT_BOLD|graphics.FONT_ANTIALIAS))
    buf.text((plots[10][0]+15,plots[10][1]+17+12), cn('请选择左键菜单中的启动连接') ,fill=0xffffff,font=("normal",11,None))
    buf.text((plots[10][0]+15,plots[10][1]+17+12+12), cn('选项-设置-帮助关于 可以看使用帮助'),fill=0xffffff,font=("normal",11,None))
    lyricBlit(currLrc)
    redraw()
...
def redrawPosition():
    ...
    buf.text((10+1,plots[20] +9+1), cn(readableTime2(position / 1000)),fill=0x000000,font=("normal",11,None))
    buf.text((mainCanvas.size[0] - 10 - 20 +1,plots[20] +9+1), u'-' + cn(readableTime2((duration - position) / 1000)),fill=0x000000,font=("normal",11,None))
    buf.text((mainCanvas.size[0] / 2 - 10+1,plots[20] +9+1),cn(readableTime2((duration ) / 1000)),fill=0x000000,font=("normal",11,None))
    
    buf.text((10,plots[20] +9), cn(readableTime2(position / 1000)),fill=0xffffff,font=("normal",11,None))
    buf.text((mainCanvas.size[0] - 10 - 20,plots[20] +9), u'-' + cn(readableTime2((duration - position) / 1000)),fill=0xffffff,font=("normal",11,None))
    buf.text((mainCanvas.size[0] / 2 - 10,plots[20] +9),cn(readableTime2((duration ) / 1000)),fill=0xffffff,font=("normal",11,None))
...
def aboutText():
    appuifw.query(cn('网易云音乐Python S60 Alpha 0.0.5版\nBy 顺'),'query')
