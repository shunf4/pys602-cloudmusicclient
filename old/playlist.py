...
def procLyric(hasLyric, testParam = False):
    global re1, re2
    #先用正则将没用的部分去掉，并用\n分割，留下纯歌词
    if testParam:
        trimLrc = re1.findall('[mark]' + hasLyric)[0].split(u'\n')
    else:
        trimLrc = re2.findall(hasLyric)
    lrcList = []
...
def procTLyric(lrcDict, hasTLyric, testParam = False):
    global re1, re2
    if testParam:
        trimTLrc = re1.findall('[mark]' + hasTLyric)[0].split(u'\n')
    else:
        trimTLrc = re2.findall(hasTLyric)
    ...
    def saveSong(self):
        try:
            ...
            if os.path.exists(self.songPath):
                desPath = 'E:\\NetEaseMusic\\%s - %s.mp3' % (cn(songDict['name']), cn(songDict['artists'][0]['name']))
                desPath = cn(desPath)
                orgPath = cn(self.songPath)
                #fileman.file_copy(orgPath, desPath, cfileman.EOverWrite)
                prompt += cn('\n此功能暂不支持！\n')
                prompt += cn('在E:\\NetEaseMusic') + cn('创建了') + cn(os.path.basename(nc(desPath))) + cn('(') + cn(str(self.br / 1000)) + cn('Kbps)')
            else:
                return False
            if self.hasLyric:
                desPath = 'E:\\NetEaseMusic\\%s - %s.lrc' % (cn(songDict['name']), cn(songDict['artists'][0]['name']))
