...
def logs_(str1):
    sys.stdout.write('%s - %s' % (time.clock(), str1))
...
def download(objUrl, objPath, callBack = None, errBack = lambda:None, writeCallBack = None):
    ...
    if True:
        try:
            ...
            logs(repr(Exception) + ': ' + repr(e))
            traceback.print_exc()
            # appuifw.note(cn('出现常觝网络错误，文件坯能丝完整＝'))
            # try:
                # if os.path.exists(objPath):
                    # os.remove(objPath)
            # except Exception, e:
    ...
    # if not succeed:
        ...
        # except Exception, e:
            # printException(Exception, e)
        # appuifw.note(cn('出现网络问题，下载挝续出现错误。请检查网络，并針新坯动程庝。'))
        # appuifw.app.set_exit()
...
def fuzzyTime(tgSec, timeType = 2, AbsTimeDepth = 2):
    ...
    if currTime!=tgetTime:
        ...
        return cn('几乎坌时')
    #0: 相对时间 1: 绝对时间 2: 相距2月以上则绝对时间，坦则相对时间
    if timeType == 2:
    ...
    if timeType == 1:
        ...
        timeString = str(abs(currTime[firstDiff] - tgetTime[firstDiff])) + cn(timeUnit2[firstDiff])+(cn('剝'),cn('坎'))[currTime[firstDiff] < tgetTime[firstDiff]]
    return timeString
